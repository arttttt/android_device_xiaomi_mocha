...
static std::vector<std::string> get_so_paths() {
    /*
     * mocha: the list is here rather than in a file on the device.
     *
     * Upstream reads it from /vendor/etc/sensors/hals.conf, which makes sense
     * for a multihal that several boards share. This one is built for this
     * board, the list has been a single line since it was imported in 2017,
     * and a configuration file that can go missing is one more way for the
     * sensors to be absent with no explanation. libsensors.lsm6db0.so is a
     * prebuilt: the IMU and the magnetometer are its, and so is the ambient
     * light sensor, despite the name.
     */
    return std::vector<std::string>{ "libsensors.lsm6db0.so" };
}
...
/*
 * mocha: what the prebuilt sub-HAL claims and what it delivers are not the
 * same, and the framework believes the claim.
 */
static void fix_sensor_fields(sensor_t& sensor) {
    /* Batching and flushing do not work, so report no fifo at all. */
    sensor.fifoReservedEventCount = 0;
    sensor.fifoMaxEventCount = 0;

    switch (sensor.type) {
    /* The flags the sensors documentation asks for on this type. */
    case SENSOR_TYPE_TILT_DETECTOR:
        sensor.flags = SENSOR_FLAG_WAKE_UP | SENSOR_FLAG_ON_CHANGE_MODE;
        break;
    /* A range the doze proximity check can work with. */
